 ...
 * tree-shakeable index per style. Data feeds the <dev-icon> element.
 */
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import type {DevIconData} from '@devicons-pack/angular';\nexport declare const ${name}: DevIconData;\nexport default ${name};\n`,
    );
...
console.log(`@devicons-pack/angular: generated ${total} icon data modules`);
