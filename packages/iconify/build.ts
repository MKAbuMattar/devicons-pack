/**
 * Generates one IconifyJSON collection per set (prefix devicons-pack-<set>)
...
fs.writeFileSync(
  ...
  `export const sets = ${JSON.stringify(STYLES)};\nexport const prefix = (set) => \`devicons-pack-\${set}\`;\n`,
);
fs.writeFileSync(
...
console.log(
  `@devicons-pack/iconify: ${total} icons across ${STYLES.length} collections`,
);
