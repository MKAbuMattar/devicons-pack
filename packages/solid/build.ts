...
export type DevIconProps = JSX.SvgSVGAttributes<SVGSVGElement> & {title?: string};
export type DevIconComponent = Component<DevIconProps>;
export declare const createDevIcon: (name: string, attrs: Record<string, string>, html: string) => DevIconComponent;
`,
);
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import {createDevIcon} from '@devicons-pack/solid';\nexport const ${name} = createDevIcon(${args});\nexport default ${name};\n`,
    );
    fs.writeFileSync(
      path.join(iconsDir, `${slug}.d.ts`),
      `import type {DevIconComponent} from '../../core.js';\nexport declare const ${name}: DevIconComponent;\nexport default ${name};\n`,
    );
...
console.log(`@devicons-pack/solid: generated ${total} icon components`);
