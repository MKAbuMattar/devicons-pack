...
const extras: Record<string, {tags: string[]; altnames: string[]}> = JSON.parse(
  fs.readFileSync(path.join(ASSETS, 'extras.json'), 'utf8'),
);
...
const icons = [...styleMap.keys()].sort().map((slug) => ({
  ...
  tags: extras[slug]?.tags ?? [],
  altnames: extras[slug]?.altnames ?? [],
}));
