import {type IconVariant, metadata} from '@devicons-pack/svg';
  ...
  return [
    {
      id: 'svg',
      label: 'SVG',
      code: `import svg from '@devicons-pack/svg/icons/${style}/${slug}.svg';\n\n// or resolve the path at runtime:\nimport {getIconPath} from '@devicons-pack/svg';\ngetIconPath('${slug}', '${style}');`,
    },
    {
      id: 'react',
      label: 'React',
      code: `import {${name}} from '@devicons-pack/react/${style}';\n\n<${name} width={32} />`,
    },
    {
      id: 'preact',
      label: 'Preact',
      code: `import {${name}} from '@devicons-pack/preact/${style}';\n\n<${name} width={32} />`,
    },
    {
      id: 'vue',
      label: 'Vue',
      code: `<script setup>\nimport {${name}} from '@devicons-pack/vue/${style}';\n</script>\n\n<template>\n  <${name} width="32" />\n</template>`,
    },
    {
      id: 'solid',
      label: 'Solid',
      code: `import {${name}} from '@devicons-pack/solid/${style}';\n\n<${name} width={32} />`,
    },
    {
      id: 'svelte',
      label: 'Svelte',
      code: `<script>\nimport {Icon} from '@devicons-pack/svelte';\nimport {${name}} from '@devicons-pack/svelte/${style}';\n</script>\n\n<Icon icon={${name}} width="32" />`,
    },
    {
      id: 'astro',
      label: 'Astro',
    ...
    },
    {
      id: 'angular',
      label: 'Angular',
      code: `import {DevIconDirective, register} from '@devicons-pack/angular';\nimport {${name}} from '@devicons-pack/angular/${style}';\n\nregister(${name});\n\n// template (import DevIconDirective in the component):\n<span devIcon="${slug}" iconStyle="${style}"></span>`,
    },
    {
      id: 'lit',
      label: 'Lit',
      code: `import {defineDevIcon, register} from '@devicons-pack/lit';\nimport {${name}} from '@devicons-pack/lit/${style}';\n\nregister(${name});\ndefineDevIcon();\n\n<dev-icon name="${slug}" variant="${style}"></dev-icon>`,
    },
    {
      id: 'wc',
      label: 'Web Components',
      code: `import {defineDevIcon, register} from '@devicons-pack/web-components';\nimport {${name}} from '@devicons-pack/web-components/${style}';\n\nregister(${name});\ndefineDevIcon();\n\n<dev-icon name="${slug}" variant="${style}"></dev-icon>`,
    },
    {
      id: 'alpine',
      label: 'Alpine.js',
      code: `import Alpine from 'alpinejs';\nimport devIcon, {register} from '@devicons-pack/alpine';\nimport {${name}} from '@devicons-pack/alpine/${style}';\n\nregister(${name});\nAlpine.plugin(devIcon);\n\n<span x-dev-icon="{name: '${slug}', style: '${style}'}"></span>`,
    },
    {
      id: 'htmx',
      label: 'htmx',
...
export default function Gallery({base}: {base: string}) {
  ...
  const [style, setStyle] = useState<IconVariant>('original');
    ...
    <div className="fe-gallery not-content">
      ...
      {icons.length === 0 ? (
        <p className="fe-empty">
          ...
          <code> react</code> or <code>python</code>.
        </p>
      ) : (
        <>
